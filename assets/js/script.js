...
async function showApi(n) {
  ...
  <div class="cardContent">
    <div>
      ...
      <div class=" position-absolute homePreview" onclick="playA('${music[i].preview}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}')">
	...
	function SearchPage() {
		...
		showcard.innerHTML += `<div  class="card border-0 resultCard">
            ...
            <div class="position-absolute preview" onclick="playA('${music[i].preview}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}'); getAudioObj('${music[i].preview}')">
            <i class="bi bi-play-fill"></i>
            </div>
            </div>
            <div class="card-body">
        ...
        <img src="${music[i].album.cover_medium}" class="card-img-top" alt="album" onclick="albumPage('${music[i].artist.name}')">  
            <div class=" position-absolute preview" onclick="playA('${music[i].preview}'); setNameArtistSong('${music[i].artist}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}')">
            <i class="bi bi-play-fill"></i>
            </div>
        </div>
        <div class="card-body">
...
async function returnApi(newA) {
    ...
    <p class="fs-6 mt-5">18,375,540 monthly listener</p>`;
		...
		follow.innerHTML += `<p><img class="w-25 position-absolute preview" src="" alt="" onclick="playA('${artista[i].preview}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}')">
    <button id="followBtn">Follow</button>`;
      ...
      <th scope="row" onclick="playA('${
			artista[i].preview
		}'); setNameArtistSong('${music[i].artist.name}', '${
				music[i].title
			}', '${music[i].album.cover_medium}','${music[i].preview}', '${
				music[i].album.title
			}')">${[i]}</th>
    ...
    <img src="${artista[i].album.cover_medium}" class="card-img-top" alt="album" onclick="albumPage('${artista[i].artist.name}')">  
        <div class="position-absolute preview" onclick="playA('${artista[i].preview}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}')">
        <i class="bi bi-play-fill"></i>
        </div>
    </div>
    <div class="card-body">
...
async function returnAlbumApi(newA) {
  ...
  <p><img class="rounded-circle coverArtist me-2" src="${album[i].artist.picture_small}" alt="Artista"><span class="fw-bold"> ${album[i].artist.name}</span> &#183; ${anno} &#183; 24 songs,<span id="text-gray">49 min 15 sec</span></p></div>`;
    ...
    <div class=" albumPreview" onclick="playA('${album[1].preview}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}')">
        <i class="bi bi-play-fill"></i>
        </div>
      <div id="heart">  
    ...
    <th scope="row" onclick="playA('${album[i].preview}'); setNameArtistSong('${
				music[i].artist.name
			}', '${music[i].title}', '${music[i].album.cover_medium}','${
				music[i].preview
			}', '${music[i].album.title}')">${[i]}</th>
  ...
  <img src="${album[i].album.cover_medium}" class="card-img-top" alt="album" onclick="albumPage('${album[i].artist.name}')">  
      <img class="w-25 position-absolute preview" src="./assets/img/play-button.png" alt="" onclick="playA('${album[i].preview}'); setNameArtistSong('${music[i].artist.name}', '${music[i].title}', '${music[i].album.cover_medium}','${music[i].preview}', '${music[i].album.title}')">
  </div>
  <div class="card-body">
...
// Funzione che trasforma l'icona play in pause e viceversa
...
// Funzione che trasforma l'icona audio in muto e viceversa
...
// Coloro e decoloro gli altri pultanti del player
...
// Apre il modale che mostra i dispositivi disponibili
...
function setNameArtistSong(artist, song, coverImg, audioPreview, titleAlbum) {
	...
	addTracksCoda(artist, song, coverImg, audioPreview, titleAlbum);
...
function addTracksCoda(artist, song, coverImg, audioPreview, titleAlbum) {
...
// Funzione che mostra i brani nella sezione Coda
function showFavoriteSongs() {
	...
	let blockerElement = document.querySelector("#blocker");
	let albumPage = document.querySelector("#albumPage");
	...
	blockerElement.classList.toggle("d-none");
	// albumPage.classList.toggle("d-none");
	// artistPage.classList.toggle("d-none");
